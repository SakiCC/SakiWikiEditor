		...
		for ( type in data ) {
			switch ( type ) {
				...
				case 'templates':
					...
					var $templates = context.modules.toolbar.$toolbar.find(
						'div[rel="' + data.section + '"].section ' + 'div[rel="' + data.page + '"].page div'
					);
					var actions = $templates.data( 'actions' );
					for ( i = 0; i < data.templates.length; i++ ) {
						// Character
						$templates
						.append(
							$( $.wikiEditor.modules.toolbar.fn.buildTemplate( data.templates[i], actions ) )
...
fn: {
	...
	buildPage : function( context, id, page ) {
		...
		switch ( page.layout ) {
			case 'table':
				...
				break;
			case 'templates':
				$page.addClass( 'page-templates' );
				var $templates = $( '<div/>' ).data( 'context', context ).data( 'actions', {} );
				var actions = $templates.data( 'actions' );
				if ( 'direction' in page ) {
					$templates.attr( 'dir', page.direction );
				} else {
					// By default it should be explicit ltr for all scripts.
					// Without this some conjoined ltr characters look
					// weird in rtl wikis.
					$templates.attr( 'dir', 'ltr' );
				}
				if ( 'templates' in page ) {
					html = '';
					for ( var i = 0; i < page.templates.length; i++ ) {
						html += $.wikiEditor.modules.toolbar.fn.buildTemplate( page.templates[i], actions );
					}
					$templates
						.html( html )
						.children()
	...
	buildTemplate : function( template, actions ) {
		if ( typeof template == 'string' ) {
			if (template == '--') return '<div class="br" />';
			if (template == '----') return '<div class="hr" />';
			if (template == '||') return '<span class="split"></span>';
			template = { 'labelMsg': template };
			return '<span class="tips">'+ $.wikiEditor.autoMsg(template, 'label') +'</span>';
		} else {
			template = {
				'labelMsg' : $.wikiEditor.autoMsg( template, ['labelMsg']),
				'titleMsg' : $.wikiEditor.autoMsg( template, ['titleMsg','labelMsg']),
				'action' : {
					'type' : 'encapsulate',
					'options' : {
						'pre' : $.wikiEditor.autoMsg( template, ['pre']),
						'preMsg' : $.wikiEditor.autoMsg( template, ['preMsg']),
						'peri' : $.wikiEditor.autoMsg( template, ['peri']),
						'periMsg' : $.wikiEditor.autoMsg( template, ['periMsg']),
						'post' : $.wikiEditor.autoMsg( template, ['post']),
						'postMsg' : $.wikiEditor.autoMsg( template, ['postMsg'])
					}
				}
			};
			if ( template && 'action' in template && 'labelMsg' in template ) {
				actions[template.labelMsg] = template.action;
				if ('titleMsg' in template)  	{
					return '<span rel="' + template.labelMsg + '" title="' + $.wikiEditor.autoMsg(template, 'title') + '">' +$.wikiEditor.autoMsg(template, 'label') + '</span>';
				} else {
					return '<span rel="' + template.labelMsg + '" title="' + $.wikiEditor.autoMsg(template, 'label') + '">' +$.wikiEditor.autoMsg(template, 'label') + '</span>';
				}
			}
		}
		mw.log( "A template for the toolbar was undefined. This is not supposed to happen. Double check the config." );
